...
namespace if2ktool
{
    public static class Consts
    {
        ...
        public const string PLIST_KEY_GENRE         = "Genre";
        public const string PLIST_KEY_KIND          = "Kind";
        public const string PLIST_KEY_TRACK_NUMBER  = "Track Number";
        public const string PLIST_KEY_TRACK_COUNT   = "Track Count";
        public const string PLIST_KEY_DISC_NUMBER   = "Disc Number";
        public const string PLIST_KEY_DISC_COUNT    = "Disc Count";
        public const string PLIST_KEY_YEAR          = "Year";
        public const string PLIST_KEY_COMMENTS      = "Comments";
        ...
        // Common ID3v2 frame names (see http://id3.org/id3v2.4.0-frames for more info)
        public const string ID3v2_FRAME_TITLE = "TIT2";
        public const string ID3v2_FRAME_ARTIST = "TPE1";
        public const string ID3v2_FRAME_ALBUM_ARTIST = "TPE2";
        public const string ID3v2_FRAME_ALBUM = "TALB";
        public const string ID3v2_FRAME_GENRE = "TCON";
        public const string ID3v2_FRAME_DATE = "TDRC";
        public const string ID3v2_FRAME_TRACK_NUMBER = "TRCK";
        public const string ID3v2_FRAME_DISC_NUMBER = "TPOS";

        // Common RIFF INFO tag IDs (see https://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/RIFF.html)
        public const string RIFF_ID_TITLE = "INAM";
        public const string RIFF_ID_ARTIST = "IART";
        public const string RIFF_ID_ALBUM = "IPRD";
        public const string RIFF_ID_GENRE = "IGNR";
        public const string RIFF_ID_COMMENTS = "ICMT";
        public const string RIFF_ID_DATE = "ICRD";
        public const string RIFF_ID_YEAR = "YEAR";
        public const string RIFF_ID_TRACK_NUMBER = "ITRK";
...
Note that this may have a significant performance impact when logging in the same thread, especially when ""Full logging"" is enabled. Consider enabling ""Log in separate thread"".";

        public const string TOOLTIP_THREADED_LOGGING =
@"Performs all logging in a separate thread.

This greatly speeds up processing time, but means that the logging is no longer in sync with the process, which has a side-effect of not being able to pause the application by selecting the log.";
...
@"(Default: true) Whenever an error occurs while matching or writing tags, pause the worker temporarily and take the action below before continuing. This can help identify issues as they come up, especially with a fast-scrolling console. Note that this doesn't affect pre or post process errors.

None - Do nothing.
Wait - Pause the worker for a few moments, enough to make it obvious, then continue.
Prompt - Print the error to a message box, will wait until the message box is dismissed before continuing the worker.";

        public const string TOOLTIP_PROGRESS_REPORT_INTERVAL =
@"(Default: 20) The interval, in milliseconds, between updating the main form UI (e.g. the progress bar and DataGridView selection) from the worker thread.

Note that setting this to very low values can and will have a performance impact, as the UI will be redrawn more often.";

        public const string TOOLTIP_PROPAGATE_CHECK_EDITS =
@"(Default: false) When true, changing the ""checked"" state of an entry while multiple entries are selected will propagate the checked state to the other rows.";
...
Having this disabled prevents accidentally changing the row height when selecting rows, which is easy to do with a narrow row height.";

        public const string TOOLTIP_ANY_EXTENSION =
@"(Default: false) If you wish to map to files that may have different extensions.
Firstly attempts to match files with the original XML entry extension, then looks in the same directory, choosing the first supported file with the same name.";

        public const string TOOLTIP_FUZZY =
@"(Default: false) If a file at the original/mapped path was not found, this tries to fuzzily match the original file name with other files in the same directory. See the readme for details.";

        public const string TOOLTIP_FUZZY_DONT_PROMPT =
@"(Default: false) If this is true, the mapper will never ask to confirm a fuzzily-matched entry - and will automatically accept it.";

        public const string TOOLTIP_NORMALIZE_STRINGS =
@"(Default: true) If a file at the original/mapped path was not found, we check for a file at the same path, but with the path string ""normalized"".

This converts strings to ""unicode-normalized form C"", which strips complex unicode characters such as combining diacritic marks (among other things), converting them to their original single-character form. See readme for more info.";

        public const string TOOLTIP_FORCE_ID3v2_ENCODING =
@"(Default: false) This sets the encoding used for *NEW* text information frames.
Note that UTF-8 isn't officially supported in ID3v2.3 and below, though many applications support it anyway.
...
True: When set, the encoding for newly-written frames will be forced to this encoding, regardless of ID3v2 version. Existing frames will continue to use whatever encoding they were previously using.

This setting is only really used when ""Write info tags to WAV files"" is used, as all playback statistics tags use ASCII encoding.";

        public const string TOOLTIP_FORCE_ID3v2_VERSION =
@"(Default: false) If checked, when writing tags, the ID3v2 version will be forced instead of writing the tags in the same version as the inputted file.
...
@"(Default: true) Disable to prevent genres from being written as indices in ID3v2 tags.";
        ...
        public const string TOOLTIP_MAX_PARALLEL_THREADS =
@"(Default: 4) The amount of threads that are available to the tag writer when processing. Generally the higher this is, the faster the tag writing process will go. However, this is limited by your disk IO - setting this too high may have a negative impact on performance.";

        public const string TOOLTIP_DRY_RUN =
@"(Default: false) Doesn't write anything to files but will simulate output. This is useful for previewing the output of if2ktool without making changes.";

        public const string TOOLTIP_WRITE_INFO_TO_WAV_FILES =
@"(Default: false) iTunes does not write tags to WAV files. This presents an issue with foo_playcount, as it relies on present tags in order to create a playback statistics entry. If any identifying tags are not present, foobar will skip over them, leaving gaps in your library.

If this option is enabled, (for WAV files only) the tool will check the file tags against the iTunes data, and will write the missing data to the file. This includes the following tags: Name, Artist, Album Artist, Album, Genre, Comments, Year, Disc Number, Disc Count, Track Number, Track Count.";
...
This substitutes the ""Music Folder"" portion (plus ""Music"") of the track path in the XML with a path of your choosing. Assumes that the structure of the original library folder is the same as the one provided.";
...
In this mode, the tags from the iTunes library XML are compared with the tags in the ""lookup"" json in order to match them to a file.";

        // Matching form match mode - search
        public const string TOOLTIP_MATCH_MODE_SEARCH =
@"Performs a search for each and every file in the library folder provided, matching it not based on file path and name - but instead the how the tags contained within the file match with the tags present in the XML. This mode takes the longest amount of time, and uses the most amount of memory - caching the tags of every file present at every depth to reduce repeated lookups.";

        // Matching form search modes
        public const string TOOLTIP_SEARCHMODE =
@"Search All: Recursively looks through all directories below the library folder, front-to-back. The most extensive search, but will take a lot of time.

Search Endpoint: Searches for files in the endpoint directory ONLY - assuming that it exists.

Search Front-to-Back: Looks for files in the library directory, stepping down to a specific directory until the next point in the path doesn't exist, or if we get to the endpoint.

Search Back-to-Front: Look for files at the endpoint directory, stepping up a directory if no matches were found, or if the directory doesn't exist. This stops when we get to the topmost library directory.";

        // Shown in preferences form, on lblLookupMinMatchPercentHeader
        public const string TOOLTIP_LOOKUP_MIN_MATCH_PERCENTAGE =
@"This is the minimum percentage of present and matching tags that an entry needs to have in order to be matched to a track in the JSON file, during Lookup matching. This is 50% by default.
...
Note that generating a JSON file with this script means that you will need to use the album list as the context for an exported masstagger script.";

        // Shown in the export playlist form, on chkExtendedM3u
        public const string TOOLTIP_EXTENDED_M3U =
@"If checked, the exported playlist will also contain extended M3U data, which prepends each track with the track number, artist and title.
...
This has no effect on the playlist generated in foobar2000, since it will only use the track paths.";
        ...
        // Header used for masstagger files. The first portion is likely a GUID of some sort, and the second is the amount of actions to perform
        public static readonly byte[] MASSTAGGER_HEADER = new byte[]
        {
            ...
            // 4 byte integer (Int32) - The number of actions to perform (in this case 1, a singular Input data)
            0x01, 0x00, 0x00, 0x00,
        };

        // This portion is used for each action in the script, and repeats any amount of times per action to perform
        public static readonly byte[] MASSTAGGER_ACTION = new byte[]
        {
            ...
            // 4 byte integer (Int32) - Number of bytes in the scheme + data, prefixes the data for each action
            0x00, 0x00, 0x00, 0x00,

            // The above two repeat for every action (with the scheme and data following each)
        };

        // The tag name portion of the masstagger header, (takes up 65 characters including one new line character)
        public static readonly byte[] MASSTAGGER_SCHEME_STATS = new byte[]
        {
            // Tag name portion
            // "%added_timestamp%;"
        ...
        public const string MASSTAGGER_SCHEME_STATS_STR = "%added_timestamp%;%last_played_timestamp%;%play_count%;%rating%\n";
        public const string MASSTAGGER_SCHEME_WAV_INFO = "%title%;%artist%;%album_artist%;%album%;%date%;%genre%;%tracknumber%;%discnumber%;%comments%\n";
        public const string MASSTAGGER_SCHEME_DEBUG_STR = "%title%\n";
    }
}
